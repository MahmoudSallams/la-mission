...
namespace Holmes\Modules\Header\Lib;

/**
 * Class HolmesMkdfHeaderConnector
 *
...
class HolmesMkdfHeaderConnector {
	...
	public function connect( $config = array() ) {
		do_action( 'holmes_mkdf_pre_header_connect' );
		...
		if ( ! empty( $config['affect_content'] ) ) {
			add_filter( 'holmes_mkdf_content_elem_style_attr', array( $this, 'contentMarginFilter' ) );
		}
		
		if ( ! empty( $config['affect_title'] ) ) {
			add_filter( 'holmes_mkdf_title_content_padding', array( $this, 'titlePaddingFilter' ) );
		}
		
		do_action( 'holmes_mkdf_after_header_connect' );
	}
	 ...
	 *
	 * Hooks to holmes_mkdf_title_content_padding filter
	 *
	 * @return int
	 */
