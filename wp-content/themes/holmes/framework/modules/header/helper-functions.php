...
if ( ! function_exists( 'holmes_mkdf_header_skin_class' ) ) {
	/**
	 * Function that adds header style class to body tag
	 */
	function holmes_mkdf_header_skin_class( $classes ) {
		$header_style     = holmes_mkdf_get_meta_field_intersect( 'header_style', holmes_mkdf_get_page_id() );
		$header_style_404 = holmes_mkdf_options()->getOptionValue( '404_header_style' );
	...
	add_filter( 'body_class', 'holmes_mkdf_header_skin_class' );
}

if ( ! function_exists( 'holmes_mkdf_sticky_header_behaviour_class' ) ) {
	/**
	 * Function that adds header behavior class to body tag
	 */
	function holmes_mkdf_sticky_header_behaviour_class( $classes ) {
		$header_behavior = holmes_mkdf_get_meta_field_intersect( 'header_behaviour', holmes_mkdf_get_page_id() );
	...
	add_filter( 'body_class', 'holmes_mkdf_sticky_header_behaviour_class' );
}

if ( ! function_exists( 'holmes_mkdf_menu_dropdown_appearance' ) ) {
	/**
	 ...
	 */
	function holmes_mkdf_menu_dropdown_appearance( $classes ) {
		$dropdown_menu_appearance = holmes_mkdf_options()->getOptionValue( 'menu_dropdown_appearance' );
	...
	add_filter( 'body_class', 'holmes_mkdf_menu_dropdown_appearance' );
}

if ( ! function_exists( 'holmes_mkdf_header_class' ) ) {
	/**
	 ...
	 */
	function holmes_mkdf_header_class( $classes ) {
		...
		$disable_menu_area_shadow = holmes_mkdf_get_meta_field_intersect( 'menu_area_shadow', $id ) == 'no';
		if ( $disable_menu_area_shadow ) {
			$classes[] = 'mkdf-menu-area-shadow-disable';
		}
		
		$disable_menu_area_grid_shadow = holmes_mkdf_get_meta_field_intersect( 'menu_area_in_grid_shadow', $id ) == 'no';
		if ( $disable_menu_area_grid_shadow ) {
			$classes[] = 'mkdf-menu-area-in-grid-shadow-disable';
		}
		
		$disable_menu_area_border = holmes_mkdf_get_meta_field_intersect( 'menu_area_border', $id ) == 'no';
		if ( $disable_menu_area_border ) {
			$classes[] = 'mkdf-menu-area-border-disable';
		}
		
		$disable_menu_area_grid_border = holmes_mkdf_get_meta_field_intersect( 'menu_area_in_grid_border', $id ) == 'no';
		if ( $disable_menu_area_grid_border ) {
			$classes[] = 'mkdf-menu-area-in-grid-border-disable';
		}
		
		if ( holmes_mkdf_get_meta_field_intersect( 'menu_area_in_grid', $id ) == 'yes' &&
		     holmes_mkdf_get_meta_field_intersect( 'menu_area_grid_background_color', $id ) !== '' &&
		     holmes_mkdf_get_meta_field_intersect( 'menu_area_grid_background_transparency', $id ) !== '0'
		) {
			$classes[] = 'mkdf-header-menu-area-in-grid-padding';
		}
		
		$disable_logo_area_border = holmes_mkdf_get_meta_field_intersect( 'logo_area_border', $id ) == 'no';
		if ( $disable_logo_area_border ) {
			$classes[] = 'mkdf-logo-area-border-disable';
		}
		
		$disable_logo_area_grid_border = holmes_mkdf_get_meta_field_intersect( 'logo_area_in_grid_border', $id ) == 'no';
		if ( $disable_logo_area_grid_border ) {
			$classes[] = 'mkdf-logo-area-in-grid-border-disable';
		}
		
		if ( holmes_mkdf_get_meta_field_intersect( 'logo_area_in_grid', $id ) == 'yes' &&
		     holmes_mkdf_get_meta_field_intersect( 'logo_area_grid_background_color', $id ) !== '' &&
		     holmes_mkdf_get_meta_field_intersect( 'logo_area_grid_background_transparency', $id ) !== '0'
		) {
			$classes[] = 'mkdf-header-logo-area-in-grid-padding';
		}
		
		$disable_shadow_vertical = holmes_mkdf_get_meta_field_intersect( 'vertical_header_shadow', $id ) == 'no';
		if ( $disable_shadow_vertical ) {
			$classes[] = 'mkdf-header-vertical-shadow-disable';
		}
		
		$disable_border_vertical = holmes_mkdf_get_meta_field_intersect( 'vertical_header_border', $id ) == 'no';
	...
	add_filter( 'body_class', 'holmes_mkdf_header_class' );
}

if ( ! function_exists( 'holmes_mkdf_header_area_style' ) ) {
	/**
	 * Function that return styles for header area
	 */
	function holmes_mkdf_header_area_style( $style ) {
		...
		$menu_area_background_color_rgba = holmes_mkdf_rgba_color( $menu_area_background_color, $menu_area_background_transparency );
		...
		if ( $menu_area_height !== '' ) {
			$menu_area_style['height'] = holmes_mkdf_filter_px( $menu_area_height ) . 'px !important';
		}
		...
		if ( $container_side_padding !== '' ) {
			...
			$current_style .= holmes_mkdf_dynamic_css( $menu_container_selector, $menu_container_styles );
		}
		...
		$menu_area_grid_background_color_rgba = holmes_mkdf_rgba_color( $menu_area_grid_background_color, $menu_area_grid_background_transparency );
		
		if ( ! empty( $menu_area_grid_background_color_rgba ) ) {
			$menu_area_grid_style['background-color'] = $menu_area_grid_background_color_rgba;
		}
		
		$current_style .= holmes_mkdf_dynamic_css( $menu_area_selector, $menu_area_style );
		$current_style .= holmes_mkdf_dynamic_css( $menu_area_grid_selector, $menu_area_grid_style );
		...
		if ( $dropdown_top_position !== '' ) {
			$dropdown_styles['top'] = holmes_mkdf_filter_suffix( $dropdown_top_position, '%' ) . '%';
		}
		
		$dropdown_selector = array( $class_prefix . ' .mkdf-page-header .mkdf-drop-down .second' );
		
		$current_style .= holmes_mkdf_dynamic_css( $dropdown_selector, $dropdown_styles );
		...
		$logo_area_background_color_rgba = holmes_mkdf_rgba_color( $logo_area_background_color, $logo_area_background_transparency );
		...
		if ( $logo_area_height !== '' ) {
			$logo_area_style['height'] = holmes_mkdf_filter_px( $logo_area_height ) . 'px !important';
		}
		...
		$logo_area_grid_background_color_rgba = holmes_mkdf_rgba_color( $logo_area_grid_background_color, $logo_area_grid_background_transparency );
		...
		if ( ! empty( $logo_area_style ) ) {
			$current_style .= holmes_mkdf_dynamic_css( $logo_area_selector, $logo_area_style );
		}
		
		if ( ! empty( $logo_area_grid_style ) ) {
			$current_style .= holmes_mkdf_dynamic_css( $logo_area_grid_selector, $logo_area_grid_style );
		}
		
		$current_style = apply_filters( 'holmes_mkdf_add_header_page_custom_style', $current_style, $class_prefix, $page_id ) . $style;
		
		return $current_style;
	}
	
	add_filter( 'holmes_mkdf_add_page_custom_style', 'holmes_mkdf_header_area_style' );
}