...
class HolmesMkdfLike {
	private static $instance;
	
	private function __construct() {
		add_action( 'wp_ajax_holmes_mkdf_like', array( $this, 'ajax' ) );
		add_action( 'wp_ajax_nopriv_holmes_mkdf_like', array( $this, 'ajax' ) );
	}
		...
		switch ( $action ) {
			...
			case 'get':
				...
				if ( isset( $_COOKIE[ 'mkdf-like_' . $post_id ] ) ) {
					$icon = '<i class="ion-android-favorite" aria-hidden="true"></i>';
				} else {
					$icon = '<i class="ion-android-favorite-outline" aria-hidden="true"></i>';
				}
			...
			case 'update':
				...
				$return_value = "<i class='ion-android-favorite' aria-hidden='true'></i><span>" . esc_attr( $like_count ) . "</span>";
	...
	public function add_like() {
		...
		$title       = esc_html__( 'Like this', 'holmes' );
		
		if ( isset( $_COOKIE[ 'mkdf-like_' . $post->ID ] ) ) {
			$class = 'mkdf-like liked';
			$title = esc_html__( 'You already like this!', 'holmes' );
		}
...
if ( ! function_exists( 'holmes_mkdf_create_like' ) ) {
	function holmes_mkdf_create_like() {
		HolmesMkdfLike::get_instance();
	}
	
	add_action( 'after_setup_theme', 'holmes_mkdf_create_like' );
}