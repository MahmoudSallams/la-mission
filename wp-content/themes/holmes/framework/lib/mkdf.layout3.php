...
/*
   Class: HolmesMkdfMultipleImages
   A class that initializes Mikado Multiple Images
*/

class HolmesMkdfMultipleImages implements iHolmesMkdfRender {
	private $name;
	private $label;
	private $description;
	...
	function __construct( $name, $label = "", $description = "" ) {
		...
		$holmes_Framework->mikadoMetaBoxes->addOption( $this->name, "" );
	}
					...
					<div class="row">
						<div class="col-lg-12">
							...
							<div class="mkdf-gallery-uploader">
								<a class="mkdf-gallery-upload-btn btn btn-sm btn-primary" href="javascript:void(0)"><?php esc_html_e( 'Upload', 'holmes' ); ?></a>
								<a class="mkdf-gallery-clear-btn btn btn-sm btn-default pull-right" href="javascript:void(0)"><?php esc_html_e( 'Remove All', 'holmes' ); ?></a>
							</div>
						</div>
					</div>
				</div>
...
class HolmesMkdfTwitterFramework implements iHolmesMkdfRender {
	public function render( $factory ) {
		$twitterApi = HolmesTwitterApi::getInstance();
		$message    = '';
		...
		if ( ! empty( $_GET['oauth_token'] ) && ! empty( $_GET['oauth_verifier'] ) ) {
			...
			if ( $responseObj->status ) {
				$message = esc_html__( 'You have successfully connected with your Twitter account. If you have any issues fetching data from Twitter try reconnecting.', 'holmes' );
			} else {
				$message = $responseObj->message;
			}
		}
		
		$buttonText = $twitterApi->hasUserConnected() ? esc_html__( 'Re-connect with Twitter', 'holmes' ) : esc_html__( 'Connect with Twitter', 'holmes' );
		?>
		<?php if ( $message !== '' ) { ?>
		...
		<div class="mkdf-page-form-section" id="mkdf_enable_social_share">
			<div class="mkdf-field-desc">
				<h4><?php esc_html_e( 'Connect with Twitter', 'holmes' ); ?></h4>
				<p><?php esc_html_e( 'Connecting with Twitter will enable you to show your latest tweets on your site', 'holmes' ); ?></p>
			</div>
			<div class="mkdf-section-content">
				<div class="container-fluid">
					<div class="row">
...
class HolmesMkdfInstagramFramework implements iHolmesMkdfRender {
	public function render( $factory ) {
		$instagram_api = HolmesInstagramApi::getInstance();
		$message       = '';
		
		//if code wasn't saved to database
		if ( ! get_option( 'holmes_instagram_code' ) ) {
			...
			if ( ! empty( $_GET['code'] ) ) {
				...
				$message = esc_html__( 'You have successfully connected with your Instagram account. If you have any issues fetching data from Instagram try reconnecting.', 'holmes' );
		...
		$buttonText = $instagram_api->hasUserConnected() ? esc_html__( 'Re-connect with Instagram', 'holmes' ) : esc_html__( 'Connect with Instagram', 'holmes' );
		?>
		<?php if ( $message !== '' ) { ?>
			<div class="alert alert-success" style="margin-top: 20px;">
				<span><?php echo esc_html( $message ); ?></span>
		...
		<div class="mkdf-page-form-section" id="mkdf_enable_social_share">
			<div class="mkdf-field-desc">
				<h4><?php esc_html_e( 'Connect with Instagram', 'holmes' ); ?></h4>
				<p><?php esc_html_e( 'Connecting with Instagram will enable you to show your latest photos on your site', 'holmes' ); ?></p>
			</div>
			<div class="mkdf-section-content">
				<div class="container-fluid">
					<div class="row">
...
class HolmesMkdfRepeater implements iHolmesMkdfRender {
	private $label;
	private $description;
	private $name;
	private $fields;
			...
			<?php if ( $this->table_layout ) { ?>
				<div class="mkdf-repeater-table-heading">
					<div class="mkdf-repeater-fields-holder">
						<div class="mkdf-repeater-table-cell mkdf-repeater-sort"><?php esc_html_e( 'Order', 'holmes' ) ?></div>
						...
						<div class="mkdf-repeater-table-cell mkdf-repeater-remove"><?php esc_html_e( 'Remove', 'holmes' ) ?></div>
					</div>
				</div>
			<?php } ?>
																			...
																			<div class="mkdf-repeater-inner-fields-row <?php echo esc_attr( $col_width_inner_class ); ?>">
																				<div class="mkdf-repeater-inner-fields-row-inner">
																					...
																					if ( ! empty( $field_inner['dependency'] ) ) {
																						$dependencyValues = holmes_mkdf_return_repeater_dependency_options_array(array(
																							'field' 	   => $field,
...
class HolmesMkdfFieldAddress extends HolmesMkdfFieldType {
					...
					<div class="row">
						<div class="col-lg-<?php echo esc_attr( $col_width ); ?>">
							...
							if ( empty( $google_maps_api_key ) ) { ?>
								<p class="description"><?php esc_html_e( 'In order for the map functionality to be enabled please input the Google Map API key in the General section of the Holmes Options', 'holmes' ); ?></p>
							<?php } ?>
...
class HolmesMkdfFieldIcon extends HolmesMkdfFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $repeat = array() ) {
		$class = '';
		...
		$icon_packs        = holmes_mkdf_icon_collections()->getIconCollectionsEmpty();
