...
/*
   Class: HolmesMkdfFramework
   A class that initializes Mikado Framework
*/
class HolmesMkdfFramework {
	private static $instance;
	public $mikadoOptions;
	public $mikadoMetaBoxes;
	public $mikadoTaxonomyOptions;
	public $mikadoUserOptions;
	private $skin;
	
	private function __construct() {
		$this->mikadoOptions         = HolmesMkdfOptions::get_instance();
		$this->mikadoMetaBoxes       = HolmesMkdfMetaBoxes::get_instance();
		$this->mikadoTaxonomyOptions = HolmesMkdfTaxonomyOptions::get_instance();
		$this->mikadoUserOptions     = HolmesMkdfUserOptions::get_instance();
		$this->mikadoDashboardOptions = HolmesMkdfDashboardOptions::get_instance();
	}
	...
	public function setSkin( HolmesMkdfSkinAbstract $skinObject ) {
		$this->skin = $skinObject;
	}
}

/**
 * Class HolmesMkdfSkinManager
 *
...
class HolmesMkdfSkinManager {
	...
	/**
	 * @see HolmesMkdfSkinManager::setSkin()
	 */
	...
	private function setSkin( $skinName = MIKADO_PROFILE_SLUG ) {
		if ( $skinName !== '' ) {
			if ( file_exists( get_template_directory() . '/framework/admin/skins/' . $skinName . '/skin.php' ) ) {
				...
				$skinName = ucfirst( $skinName ) . esc_html__( 'Skin', 'holmes' );
...
/**
 * Class HolmesMkdfSkinAbstract
 *
 * Abstract class that each skin class must extend
 */
abstract class HolmesMkdfSkinAbstract {
	/**
	 * @var string
	 */
	protected $skinName;
	...
	public function loadTemplatePart( $template, $params = array() ) {
		...
		if ( $template !== '' ) {
			include( holmes_mkdf_find_template_path( 'framework/admin/skins/' . $this->skinName . '/templates/' . $template . '.php' ) );
		}
	}
	...
	public function setShortcodeJSParams() { ?>
		<script>
			window.mikadoSCIcon = '<?php echo holmes_mkdf_get_skin_uri() . '/assets/img/admin-logo-icon.png'; ?>';
			window.mikadoSCLabel = '<?php echo esc_html( ucfirst( $this->skinName ) ); ?> <?php esc_html_e( 'Shortcodes', 'holmes' ) ?>';
		</script>
	<?php }
...
class HolmesMkdfOptions {
	...
	public function getOptionValue( $key ) {
		...
		if ( array_key_exists( $key, $holmes_options ) ) {
			return $holmes_options[ $key ];
...
/*
   Class: HolmesMkdfAdminPage
   A class that initializes Mikado Admin Page
*/
class HolmesMkdfAdminPage implements iHolmesMkdfLayoutNode {
	public $layout;
	private $factory;
	public $slug;
	public $title;
	...
	function __construct( $slug = "", $title = "", $icon = "" ) {
		$this->layout  = array();
		$this->factory = new HolmesMkdfFieldFactory();
		$this->slug    = $slug;
		$this->title   = $title;
		$this->icon    = $icon;
	}
	...
	public function renderChild( iHolmesMkdfRender $child ) {
		$child->render( $this->factory );
	}
}

/*
   Class: HolmesMkdfMetaBoxes
...
/*
   Class: HolmesMkdfMetaBox
   A class that initializes Mikado Meta Box
*/
class HolmesMkdfMetaBox implements iHolmesMkdfLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	public $title;
	...
	public function setTitle( $label ) {
		...
		return $holmes_Framework->getSkin()->getSkinLabel() . ' ' . $label;
	}
}

/*
   Class: HolmesMkdfTaxonomyOptions
   A class that initializes HolmesMkdf Taxonomy Options
*/
class HolmesMkdfTaxonomyOptions {
	private static $instance;
...
/*
   Class: HolmesMkdfTaxonomyOption
   A class that initializes HolmesMkdf Taxonomy Option
*/
class HolmesMkdfTaxonomyOption implements iHolmesMkdfLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	...
	function __construct( $scope = "" ) {
		$this->layout  = array();
		$this->factory = new HolmesMkdfTaxonomyFieldFactory();
		$this->scope   = $scope;
	}
...
/*
   Class: HolmesMkdfUserOptions
   A class that initializes HolmesMkdf User Options
*/
class HolmesMkdfUserOptions {
	private static $instance;
	public $userOptions;
...
/*
   Class: HolmesMkdfUserOption
   A class that initializes HolmesMkdf User Option
*/
class HolmesMkdfUserOption implements iHolmesMkdfLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	...
	function __construct( $scope = "" ) {
		$this->layout  = array();
		$this->factory = new HolmesMkdfUserFieldFactory();
		$this->scope   = $scope;
	}
...
/*
   Class: HolmesMkdfDashboardOptions
   A class that initializes HolmesMkdf Dashboard Options
*/
class HolmesMkdfDashboardOptions {
	private static $instance;
...
/*
   Class: HolmesMkdfDashboardOption
   A class that initializes HolmesMkdf Dashboard Option
*/
class HolmesMkdfDashboardOption implements iHolmesMkdfLayoutNode {
	public $layout;
	private $factory;
	
	function __construct() {
		$this->layout        = array();
		$this->factory       = new HolmesMkdfDashboardFieldFactory();
	}
...
if ( ! function_exists( 'holmes_mkdf_init_framework_variable' ) ) {
	function holmes_mkdf_init_framework_variable() {
		global $holmes_Framework;
		
		$holmes_Framework = HolmesMkdfFramework::get_instance();
		$mikadoSkinManager                 = new HolmesMkdfSkinManager();
		$holmes_Framework->setSkin( $mikadoSkinManager->getSkin() );
	}
	
	add_action( 'holmes_mkdf_before_options_map', 'holmes_mkdf_init_framework_variable' );
}
?>