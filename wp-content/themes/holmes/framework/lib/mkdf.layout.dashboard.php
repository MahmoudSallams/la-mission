...
/*
   Class: HolmesMkdfDashboardForm
   A class that initializes Mikado Dashboard Container
*/

class HolmesMkdfDashboardForm implements iHolmesMkdfLayoutNode, iHolmesMkdfRender {
	public $children;
	public $name;
	public $form_id;
	public $form_method;
...
/*
   Class: HolmesMkdfDashboardGroup
   A class that initializes HolmesMkdf Group Field
*/

class HolmesMkdfDashboardGroup implements iHolmesMkdfLayoutNode, iHolmesMkdfRender {
	public $children;
	public $name;
	public $title;
	public $description;
...
/*
   Class: HolmesMkdfDashboardTitle
   A class that initializes Dashboard Title
*/

class HolmesMkdfDashboardTitle implements iHolmesMkdfRender {
	private $name;
	private $title;
	private $args = array();
...
/*
   Class: HolmesMkdfDashboardField
   A class that initializes HolmesMkdf Front Field
*/

class HolmesMkdfDashboardField implements iHolmesMkdfRender {
	private $type;
	private $name;
	private $label;
	private $description;
...
abstract class HolmesMkdfDashboardFieldType {
	abstract public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = "" );
}

class HolmesMkdfDashboardFieldText extends HolmesMkdfDashboardFieldType {
...
class HolmesMkdfDashboardFieldTextArea extends HolmesMkdfDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$col_width = 12;
...
class HolmesMkdfDashboardFieldImage extends HolmesMkdfDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$class = '';
            ...
            <div class="mkdf-dashboard-gallery-holder">
                <ul class="mkdf-dashboard-gallery-images-holder">
					<?php if ( isset( $value_html ) ) {
						print holmes_mkdf_get_clean_output( $value_html );
					} ?>
                </ul>
                <div class="mkdf-dashboard-gallery-uploader">
					<?php
					if ( holmes_membership_theme_installed() ) {
						echo holmes_mkdf_get_button_html( array(
							'text'         => esc_html__( 'Upload', 'holmes' ),
							'custom_class' => 'mkdf-dashboard-gallery-upload'
						) );
					} else {
						echo '<a itemprop="url" href="#" class="mkdf-btn mkdf-btn-medium mkdf-btn-solid mkdf-dashboard-gallery-upload"><span class="mkdf-btn-text">' . esc_html__( 'Upload', 'holmes' ) . '</span></a>';
					} ?>
					...
					<?php if ( $value !== '' && $value !== false ) { ?>
                        <button class="mkdf-btn mkdf-btn-solid mkdf-dashboard-remove-image"><?php esc_html_e( 'Remove Media', 'holmes' ); ?></button>
					<?php } ?>
                </div>
            </div>
        </div>
...
class HolmesMkdfDashboardFieldGallery extends HolmesMkdfDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$class = '';
...
class HolmesMkdfDashboardFieldSelect extends HolmesMkdfDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$class = '';
...
class HolmesMkdfDashboardFieldDate extends HolmesMkdfFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		if ( ! empty( $repeat ) && array_key_exists( 'name', $repeat ) && array_key_exists( 'index', $repeat ) ) {
			$id   = $name . '-' . $repeat['index'];
			$name = $repeat['name'] . '[' . $repeat['index'] . '][' . $name . ']';
...
class HolmesMkdfDashboardFieldIcon extends HolmesMkdfDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$options           = holmes_mkdf_icon_collections()->getIconCollectionsEmpty();
		$icons_collections = holmes_mkdf_icon_collections()->getIconCollectionsKeys();
                ...
                <div class="mkdf-dashboard-item">
                    ...
                    <div class="mkdf-dashboard-icon-holder">
						...
						<?php foreach ( $icons_collections as $icons_collection ) {
							$icons_param = holmes_mkdf_icon_collections()->getIconCollectionParamNameByKey( $icons_collection );
							$style       = 'display: none';
                            ...
                            <div class="mkdf-icon-collection-holder" style="<?php echo esc_attr( $style ); ?>" data-icon-collection="<?php echo esc_attr( $icons_collection ); ?>">
                                <select name="<?php echo esc_attr( $name . '[' . $icons_param . ']' ); ?>" id="<?php echo esc_attr( $name . '[' . $icons_param . ']' ); ?>">
									<?php
									$icons       = holmes_mkdf_icon_collections()->getIconCollection( $icons_collection );
...
class HolmesMkdfDashboardFieldColor extends HolmesMkdfDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		if ( ! empty( $repeat ) && array_key_exists( 'name', $repeat ) && array_key_exists( 'index', $repeat ) ) {
			$id   = $name . '-' . $repeat['index'];
			$name = $repeat['name'] . '[' . $repeat['index'] . '][' . $name . ']';
...
class HolmesMkdfDashboardFieldCheckBoxGroup extends HolmesMkdfDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$col_width = 12;
...
class HolmesMkdfDashboardFieldAddress extends HolmesMkdfFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$col_width = 12;
		if ( isset( $args["col_width"] ) ) {
			$col_width = $args["col_width"];
            ...
            <div class="mkdf-dashboard-field-row">
                <div class="mkdf-dashboard-item col-lg-<?php echo esc_attr( $col_width ); ?>">
                    <div class="mkdf-dashboard-input-holder">
                        ...
                        <a id="reset" href="#" style="display:none;"><?php esc_html_e( 'Reset Marker', 'holmes' ); ?></a>
                    </div>
                </div>
            </div>
        </div>
...
class HolmesMkdfDashboardRepeater implements iHolmesMkdfRender {
	private $label;
	private $description;
	private $name;
	private $fields;
	...
	function __construct( $fields, $name, $label = '', $description = '', $button_text = '', $table_layout = false, $value = array() ) {
		...
		$this->name         = $name;
		$this->num_of_rows  = 1;
		$this->button_text  = ! empty( $button_text ) ? $button_text : esc_html__( 'Add New Item', 'holmes' );
			...
			<?php if ( $this->table_layout ) { ?>
                <div class="mkdf-dashboard-repeater-table-heading">
                    <div class="mkdf-dashboard-repeater-fields-holder">
                        <div class="mkdf-dashboard-repeater-table-cell mkdf-dashboard-repeater-sort"><?php esc_html_e( 'Order', 'holmes' ) ?></div>
                        ...
                        <div class="mkdf-dashboard-repeater-table-cell mkdf-dashboard-repeater-remove"><?php esc_html_e( 'Remove', 'holmes' ) ?></div>
                    </div>
                </div>
			<?php } ?>
                                    ...
                                    <div class="mkdf-dashboard-repeater-fields-row <?php echo esc_attr( $col_width_class ); ?>">
                                        <div class="mkdf-dashboard-repeater-fields-row-inner">
											...
											if ( $field['type'] == 'repeater' ) {
																		...
																		foreach ( $field['fields'] as $field_inner ) {
                                                                            ...
                                                                            <div class="mkdf-dashboard-repeater-inner-fields-row <?php echo esc_attr( $col_width_inner_class ); ?>">
                                                                                <div class="mkdf-dashboard-repeater-inner-fields-row-inner">
																					...
																					$factory->render( $field_inner['type'], $field_inner['name'], $field_inner['label'], $field_inner['description'], $field_inner['options'], $field_inner['args'], $repeater_inner_field_value, array(
												...
												$factory->render( $field['type'], $field['name'], $field['label'], $field['description'], $field['options'], $field['args'], $repeater_field_value, array(
													'name'  => $this->name,
													'index' => $counter
												) );
											} ?>
...
class HolmesMkdfDashboardFieldFactory {
	public function render( $field_type, $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		switch ( strtolower( $field_type ) ) {
			case 'text':
				$field = new HolmesMkdfDashboardFieldText();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;

			case 'textarea':
				$field = new HolmesMkdfDashboardFieldTextArea();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;

			case 'date':
				$field = new HolmesMkdfDashboardFieldDate();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;

			case 'image':
				$field = new HolmesMkdfDashboardFieldImage();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;

			case 'gallery':
				$field = new HolmesMkdfDashboardFieldGallery();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;

			case 'select':
				$field = new HolmesMkdfDashboardFieldSelect();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;

			case 'icon':
				$field = new HolmesMkdfDashboardFieldIcon();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;

			case 'color':
				$field = new HolmesMkdfDashboardFieldColor();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;

			case 'checkboxgroup':
				$field = new HolmesMkdfDashboardFieldCheckBoxGroup();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;

			case 'address':
				$field = new HolmesMkdfDashboardFieldAddress();
