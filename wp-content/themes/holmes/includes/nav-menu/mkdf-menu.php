...
if ( ! function_exists( 'holmes_mkdf_add_custom_nav_fields' ) ) {
	function holmes_mkdf_add_custom_nav_fields( $menu_item ) {
...
//        var_dump($menu_item);exit;
	...
	add_filter( 'wp_setup_nav_menu_item', 'holmes_mkdf_add_custom_nav_fields' );
}
 ...
 * @return      void
 */
if ( ! function_exists( 'holmes_mkdf_update_custom_nav_fields' ) ) {
	function holmes_mkdf_update_custom_nav_fields( $menu_id, $menu_item_db_id, $args ) {
		$check = array(
			'anchor',
			'nolink',
			'hide',
	...
	// save menu custom fields
	add_action( 'wp_update_nav_menu_item', 'holmes_mkdf_update_custom_nav_fields', 10, 3 );
}
 ...
 * @return      void
 */
if ( ! function_exists( 'holmes_mkdf_edit_walker' ) ) {
	function holmes_mkdf_edit_walker( $walker, $menu_id ) {
		return 'HolmesMkdf_Walker_Nav_Menu_Edit_Custom';
	}
	
	// edit menu walker
	add_filter( 'wp_edit_nav_menu_walker', 'holmes_mkdf_edit_walker', 10, 2 );
}

if ( ! function_exists( 'holmes_mkdf_include_custom_walkers' ) ) {
	function holmes_mkdf_include_custom_walkers() {
		...
		do_action( 'holmes_mkdf_include_custom_walkers_nav' );
	}
	
	add_action( 'after_setup_theme', 'holmes_mkdf_include_custom_walkers' );
}